...
def admin():
    ...
    if form.validate_on_submit():
        ...
        mods = []
        for mod in profile.modpack:
            mods.append(mod.UGCId)

        if not set_profile(map_id=profile.map.id, gamemode_id=profile.gamemode.id, mods=mods):
            logger.fatal(f"Failed to set profile {profile.name} {profile.name}")
            flash(f"Failed to set profile  {profile.name} {profile.name}")
            return redirect(url_for('admin'))

        flash("Set profile success!")
        return redirect(url_for('admin'))
